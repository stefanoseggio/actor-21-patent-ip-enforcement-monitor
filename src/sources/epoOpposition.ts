...
export async function getOpsAccessToken(consumerKey: string, consumerSecret: string): Promise<string> {
    const basicAuth = Buffer.from(`${consumerKey}:${consumerSecret}`).toString('base64');
    const response = await fetchJsonWithRetry<OpsTokenResponse>(AUTH_URL, {
        method: 'POST',
        headers: {
...
function isOppositionRelated(code: string | null, description: string | null): boolean {
 ...
 */
export function parseLegalEvents(xml: string, publicationNumber: string): EpoLegalEvent[] {
    ...
    $('legal-event, event').each((_, el) => {
        const node = $(el);
        const code =
            node.attr('code') ?? node.attr('event-code') ?? (node.find('code, event-code').first().text().trim() || null);
        const date =
            node.attr('date') ?? node.attr('event-date') ?? (node.find('date, event-date').first().text().trim() || null);
        const description =
            node.find('text, event-desc, description').first().text().trim() ||
            node.attr('desc') ||
            null;
...
export async function fetchLegalEvents(publicationNumber: string, accessToken: string): Promise<EpoLegalEvent[]> {
...
function toRawRecord(event: EpoLegalEvent, now: string): EpoRawRecord {
    return {
...
export async function fetchEpoOppositionEvents(params: FetchEpoOppositionParams): Promise<EpoRawRecord[]> {
    const now = new Date().toISOString();
    const accessToken = await getOpsAccessToken(params.consumerKey, params.consumerSecret);
    const records: EpoRawRecord[] = [];
    ...
    for (const publicationNumber of params.watchlist) {
        ...
        for (const event of events) {
            if (!event.isOppositionRelated) continue;
