 ...
 */
async function doFetchWithRetry(url, init, options, parseResponse) {
...
export async function fetchJsonWithRetry(url, init, options = {}) {
    return (await doFetchWithRetry(url, init, options, async (r) => r.json()));
}
...
export async function fetchTextWithRetry(url, init, options = {}) {
    return (await doFetchWithRetry(url, init, options, async (r) => r.text()));
}
