 ...
 */
function buildRequestBody(params, offset, limit) {
    const filters = [];
...
function flattenProceeding(proceeding, now) {
 ...
 */
export async function fetchPtabProceedings(params) {
    ...
    while (records.length < params.maxItems) {
        ...
        const page = await fetchJsonWithRetry(SEARCH_URL, {
            method: 'POST',
            headers: {
        ...
        if (proceedings.length < Math.min(limit, remaining))
            break; // last page
        offset += proceedings.length;
    }
    return records;
}
//# sourceMappingURL=usptoPtab.js.map