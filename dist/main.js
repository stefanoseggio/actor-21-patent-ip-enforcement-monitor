...
function classify(state, source, recordId, fingerprint) {
    const result = classifyRecord(state, source, recordId, fingerprint);
    if (result === 'first_seen')
        return { eventType: 'SANCTION', isNew: true, result };
    if (result === 'updated')
        return { eventType: 'UPDATED', isNew: false, result };
...
async function run() {
    ...
    try {
        if (input.sources.includes('uspto_ptab')) {
            if (!input.usptoOdpApiKey)
                throw new Error('usptoOdpApiKey is required for source uspto_ptab.');
            ...
            const fingerprintsThisRun = {};
            const terminationDatesThisRun = {};
            for (const raw of ptabRecords) {
                ...
                if (input.onlyNew && result === 'unchanged')
                    continue;
...
function dateRangeToFrom(dateRange) {
    if (!dateRange)
        return undefined;
